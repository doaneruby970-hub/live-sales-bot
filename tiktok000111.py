...
# 1. 网络代理 (如需代理请设置环境变量 http_proxy / https_proxy)
...
# 3. TikTok 目标直播间 (设置环境变量 TIKTOK_TARGET_USERNAME)
TIKTOK_TARGET_USERNAME = os.getenv("TIKTOK_TARGET_USERNAME", "@tiktok")
...
# 初始化 TikTok 客户端
tiktok_client: TikTokLiveClient = TikTokLiveClient(unique_id=TIKTOK_TARGET_USERNAME)
...
async def on_connect(event: ConnectEvent):
    print(f"✅ [TikTok] 连接成功！已进入 @{tiktok_client.unique_id} 的直播间。")
    print("👂 正在监听弹幕...")
...
async def on_comment(event: CommentEvent):
    ...
    print(f"📨 [TikTok弹幕] {user_name}: {comment_text}")

    # 放入队列，交给下面的主逻辑处理
    if comment_text.strip():
        input_queue.put(comment_text)


@tiktok_client.on(DisconnectEvent)
async def on_disconnect(event: DisconnectEvent):
    print("❌ [TikTok] 直播已结束或连接断开。")


def start_tiktok_listener():
    """在单独线程中运行 TikTok 客户端"""
    print(f"🚀 正在启动 TikTok 监听，目标: {TIKTOK_TARGET_USERNAME} (Proxy: 9674)...")
    try:
        tiktok_client.run()
    except Exception as e:
        print(f"❌ TikTok 连接错误: {e}")
    ...
    try:
        ...
        with open(REF_AUDIO_PATH, 'rb') as audio_file:
            ...
            if response.status_code == 200:
                ...
                print(f"❌ 语音服务报错: {response.status_code}")
    except Exception as e:
        print(f"❌ 语音合成失败: {e}")
    return None
...
def main():
    ...
    print("\n⌚️ 智能体已就绪 (V7.8 TikTok融合·原味Prompt版)！")

    # 2. 启动耳朵 (TikTok监听) - 替代原来的键盘输入
    t = threading.Thread(target=start_tiktok_listener, daemon=True)
    t.start()

    print("👉 可以在 TikTok 直播间发弹幕测试...")
